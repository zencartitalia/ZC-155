<?php
/**
 * Modulo di pagamento con bonifico
 *
 * @package paymentMethod
 * @copyright Copyright 2003-2016 Zen Cart Development Team
 * @copyright Portions Copyright 2003 osCommerce
 * @license http://www.zen-cart.com/license/2_0.txt GNU Public License V2.0
 * @version $Id: bonifico.php 2016-10-07 10:38 Paolo De Dionigi aka Spike00 modificato in V 1.5.5
 */
...
// class methods
    function update_status() {
      ...
      if ($this->enabled == true && (int)MODULE_PAYMENT_BONIFICO_ZONE > 0 && isset($order->billing['country']['id'])) {
        $check_flag = false;
        $check = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_BONIFICO_ZONE . "' and zone_country_id = '" . (int)$order->billing['country']['id'] . "' order by zone_id");
        while (!$check->EOF) {
